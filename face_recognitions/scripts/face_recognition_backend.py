...
from face_recognitions import knn
...
class FaceRecognitionBackend(Node):
    def __init__(self):
        ...
        #load dataset
        if len(label_name)%2!=0:
            self.face_clf = knn.NearestNeighbor("euclidean",10)
        else:
            self.face_clf = knn.NearestNeighbor("euclidean",11)
        for name in label_name:
            ...
            self.face_clf.partial_fit(np.array(data),name)
    ...
    def detect(self, frame):
        ...
        if results.detections:
            ...
            for _ in zip(roi_box, face_img):
                ...
                pred = self.face_clf.predict(embeddings)
                name_list.append(pred[0])
        self.get_logger().info(f"{pred[1]}")
